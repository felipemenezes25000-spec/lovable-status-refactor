import { Card } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { useState } from 'react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
} from '@/components/ui/dialog';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Badge } from '@/components/ui/badge';
import { Switch } from '@/components/ui/switch';
import { Label } from '@/components/ui/label';

interface Rotina {
  name: string;
  status: 'success' | 'delayed' | 'error' | 'maintenance';
  desc: string;
  last: string;
  avg: string;
  deps: string;
  start: string;
  end: string;
  feriado: boolean;
  diaUtil: boolean;
  finalSemana: boolean;
}

const rotinas: Rotina[] = [
  {
    name: 'Banking',
    status: 'success',
    desc: 'Rotina bancária de conciliações diárias: liquidação de TED/DOC/PIX, reconciliação contábil e geração de extratos para downstream.',
    last: 'Hoje 21:15 BRT',
    avg: '12 min',
    deps: 'PIX Gateway, Core Banking, Ledger',
    start: '01:00 BRT',
    end: '01:30 BRT',
    feriado: false,
    diaUtil: true,
    finalSemana: false,
  },
  {
    name: 'Energy',
    status: 'delayed',
    desc: 'Carga de posições de energia e cálculo de exposição. Consolida curvas, preços e PnL para o book de Energia.',
    last: 'Hoje 20:40 BRT',
    avg: '18 min',
    deps: 'Market Data, Risco, Data Lake',
    start: '02:00 BRT',
    end: '02:30 BRT',
    feriado: false,
    diaUtil: true,
    finalSemana: false,
  },
  {
    name: 'Payments',
    status: 'success',
    desc: 'Rotina de pagamentos massivos. Gera arquivos CNAB, valida limites e dispara lotes via SPI/STR.',
    last: 'Hoje 19:55 BRT',
    avg: '9 min',
    deps: 'Tesouraria, SPI, Compliance',
    start: '03:00 BRT',
    end: '03:20 BRT',
    feriado: false,
    diaUtil: true,
    finalSemana: false,
  },
  {
    name: 'Command Center',
    status: 'success',
    desc: 'Health-checks do Command Center e coleta de KPIs operacionais para os painéis.',
    last: 'Hoje 22:00 BRT',
    avg: '3 min',
    deps: 'Observabilidade, Status API',
    start: '00:05 BRT',
    end: '00:08 BRT',
    feriado: false,
    diaUtil: true,
    finalSemana: true,
  },
  {
    name: 'Commodities',
    status: 'success',
    desc: 'Ingestão de cotações e posições de commodities. Normaliza fontes e atualiza curvas para pricing.',
    last: 'Hoje 20:10 BRT',
    avg: '14 min',
    deps: 'Market Data, Risco',
    start: '04:00 BRT',
    end: '04:20 BRT',
    feriado: false,
    diaUtil: true,
    finalSemana: false,
  },
  {
    name: 'LUX',
    status: 'success',
    desc: 'Rotinas do booking Luxemburgo: NAV, reconciliação com custodiante e geração de reports regulatórios.',
    last: 'Hoje 18:30 CET',
    avg: '22 min',
    deps: 'Custodian, Fund Admin, Ledger',
    start: '02:00 CET',
    end: '02:30 CET',
    feriado: false,
    diaUtil: true,
    finalSemana: false,
  },
  {
    name: 'Portfolio Services',
    status: 'maintenance',
    desc: 'Carga de eventos corporativos e processamento de dividendos para carteiras de clientes.',
    last: 'Hoje 21:05 BRT',
    avg: '11 min',
    deps: 'B3, Custódia, Data Lake',
    start: '05:00 BRT',
    end: '05:15 BRT',
    feriado: false,
    diaUtil: true,
    finalSemana: false,
  },
  {
    name: 'Panorama',
    status: 'success',
    desc: 'Agregação de dados para o dashboard executivo Panorama: KPIs de disponibilidade, latência e volume.',
    last: 'Hoje 22:02 BRT',
    avg: '5 min',
    deps: 'Status API, Data Platform',
    start: 'a cada 10 min',
    end: 'contínuo',
    feriado: false,
    diaUtil: true,
    finalSemana: true,
  },
  {
    name: 'Portfolio Management',
    status: 'success',
    desc: 'Batch de cálculo de risco e rebalanceamento de portfólios segundo restrições de mandato.',
    last: 'Hoje 20:20 BRT',
    avg: '16 min',
    deps: 'Risco, Precificação, OMS',
    start: '06:00 BRT',
    end: '06:30 BRT',
    feriado: false,
    diaUtil: true,
    finalSemana: false,
  },
];

const RotinasTab = () => {
  const [selectedRotina, setSelectedRotina] = useState<Rotina | null>(null);
  const [editedStatus, setEditedStatus] = useState<string>('');
  const [editedFeriado, setEditedFeriado] = useState(false);
  const [editedDiaUtil, setEditedDiaUtil] = useState(false);
  const [editedFinalSemana, setEditedFinalSemana] = useState(false);

  const openDialog = (rotina: Rotina) => {
    setSelectedRotina(rotina);
    setEditedStatus(rotina.status);
    setEditedFeriado(rotina.feriado);
    setEditedDiaUtil(rotina.diaUtil);
    setEditedFinalSemana(rotina.finalSemana);
  };

  const getStatusBadge = (status: string) => {
    const config = {
      success: { label: 'Concluído', variant: 'default' as const, className: 'bg-emerald-500 hover:bg-emerald-600' },
      delayed: { label: 'Com Atraso', variant: 'secondary' as const, className: 'bg-amber-500 hover:bg-amber-600 text-white' },
      error: { label: 'Erro', variant: 'destructive' as const, className: '' },
      maintenance: { label: 'Manutenção', variant: 'outline' as const, className: 'border-blue-500 text-blue-600 dark:text-blue-400' },
    };
    const { label, variant, className } = config[status as keyof typeof config] || config.success;
    return <Badge variant={variant} className={className}>{label}</Badge>;
  };

  return (
    <>
      <Card className="p-4 shadow-lg">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold">CCBP - Rotinas de Produção</h2>
            <p className="text-sm text-muted-foreground">Status das fases por cliente e segmento</p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm">Recarregar</Button>
            <Button variant="outline" size="sm">Filtros</Button>
          </div>
        </div>

        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {rotinas.map((rotina, idx) => (
            <Card
              key={idx}
              className="cursor-pointer p-3 transition-all hover:shadow-xl hover:scale-[1.02] hover:-translate-y-1"
              onClick={() => openDialog(rotina)}
            >
              <div className="mb-2 flex items-center justify-between">
                <div className="font-bold">{rotina.name}</div>
                {getStatusBadge(rotina.status)}
              </div>
              <div className="space-y-1 text-xs text-muted-foreground">
                <div>Última: {rotina.last}</div>
                <div>Duração média: {rotina.avg}</div>
              </div>
            </Card>
          ))}
        </div>
      </Card>

      <Dialog open={!!selectedRotina} onOpenChange={() => setSelectedRotina(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center justify-between">
              <span>{selectedRotina?.name}</span>
              {selectedRotina && getStatusBadge(editedStatus)}
            </DialogTitle>
            <DialogDescription className="mt-3 space-y-4 text-left">
              <p className="text-sm text-foreground">{selectedRotina?.desc}</p>
              
              {/* Status Selection */}
              <div className="rounded-lg border bg-card p-4">
                <Label className="text-sm font-semibold">Alterar Status da Rotina</Label>
                <Select value={editedStatus} onValueChange={setEditedStatus}>
                  <SelectTrigger className="mt-2">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="success">Concluído com Sucesso</SelectItem>
                    <SelectItem value="delayed">Concluído com Atraso</SelectItem>
                    <SelectItem value="error">Erro / Não Rodou</SelectItem>
                    <SelectItem value="maintenance">Em Manutenção</SelectItem>
                  </SelectContent>
                </Select>
              </div>

              {/* Execution Criteria */}
              <div className="rounded-lg border bg-card p-4 space-y-3">
                <Label className="text-sm font-semibold">Critérios de Execução</Label>
                
                <div className="flex items-center justify-between">
                  <Label htmlFor="feriado" className="text-sm">Executar em Feriados</Label>
                  <Switch
                    id="feriado"
                    checked={editedFeriado}
                    onCheckedChange={setEditedFeriado}
                  />
                </div>
                
                <div className="flex items-center justify-between">
                  <Label htmlFor="diaUtil" className="text-sm">Executar em Dias Úteis</Label>
                  <Switch
                    id="diaUtil"
                    checked={editedDiaUtil}
                    onCheckedChange={setEditedDiaUtil}
                  />
                </div>
                
                <div className="flex items-center justify-between">
                  <Label htmlFor="finalSemana" className="text-sm">Executar em Final de Semana</Label>
                  <Switch
                    id="finalSemana"
                    checked={editedFinalSemana}
                    onCheckedChange={setEditedFinalSemana}
                  />
                </div>
              </div>
              
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-lg border bg-muted/50 p-3">
                  <div className="text-xs font-semibold text-muted-foreground">Última execução</div>
                  <div className="text-sm font-medium">{selectedRotina?.last}</div>
                </div>
                
                <div className="rounded-lg border bg-muted/50 p-3">
                  <div className="text-xs font-semibold text-muted-foreground">Duração média</div>
                  <div className="text-sm font-medium">{selectedRotina?.avg}</div>
                </div>
                
                <div className="rounded-lg border bg-muted/50 p-3">
                  <div className="text-xs font-semibold text-muted-foreground">Início programado</div>
                  <div className="text-sm font-medium">{selectedRotina?.start}</div>
                </div>
                
                <div className="rounded-lg border bg-muted/50 p-3">
                  <div className="text-xs font-semibold text-muted-foreground">Fim estimado</div>
                  <div className="text-sm font-medium">{selectedRotina?.end}</div>
                </div>
              </div>
              
              <div className="rounded-lg border bg-muted/50 p-3">
                <div className="text-xs font-semibold text-muted-foreground">Dependências</div>
                <div className="text-sm font-medium">{selectedRotina?.deps}</div>
              </div>
            </DialogDescription>
          </DialogHeader>
          
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setSelectedRotina(null)}>
              Fechar
            </Button>
            <Button onClick={() => {
              // Aqui salvaria as alterações
              setSelectedRotina(null);
            }}>
              Salvar Alterações
            </Button>
          </div>
        </DialogContent>
      </Dialog>
    </>
  );
};

export default RotinasTab;
